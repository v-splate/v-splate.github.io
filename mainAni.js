...
let currentFrame=0;
let maxFrame=100; 
const AnimationTimeInterval=1000/animationfps;
let lastAnimationTime=0;
var AnimationChunks = [];
let frameoffset = 0;
let isAnimation=false;
let isAllFrameLoaded=false;
...
async function readChunksFromFile(file) {
    ...
    while (offset < dataView.byteLength) {
      if (offset + 4 > dataView.byteLength) {
        throw new Error("Unexpected end of file while reading length");
      }
      
      const length = dataView.getUint32(offset, false); // false = big-endian
      offset += 4;
  
      if (offset + length > dataView.byteLength) {
        throw new Error("Unexpected end of file while reading data chunk");
      }
...
async function main() {
    ...
    window.addEventListener("keydown", (e) => {
        ...
        if (e.code == "KeyV") {
            ...
            if(isVideoPlaying==false)
                camid.innerText = "Video paused, press P key to play";
            else
                camid.innerText = "";
        }
    });
    ...
    const frame = (now) => {
        ...
        if(isAnimation && isVideoPlaying){
            const elapsed =now-lastAnimationTime; 
            if(elapsed>=AnimationTimeInterval){
                lastAnimationTime=now-(elapsed%AnimationTimeInterval);
                if(isAllFrameLoaded){
                    currentFrame++;
                    ...
                    worker.postMessage({ 
                        buffer: AnimationChunks[currentFrame].buffer, //splatData.buffer,
                        vertexCount: Math.floor(AnimationChunks[currentFrame].length / rowLength),
                    });

                }else{
                    readChunksByID();   
                }
                //console.log("currentFrame "+currentFrame+ " maxframe "+maxFrame)    
            } 
        }
        lastFrame = now;
    ...
    const selectFile = (file) => {
        ...
        if (/\.json$/i.test(file.name)) {
            ...
            stopLoading = true;
            fr.onload = () => {  
                if (/\.splatv$/i.test(file.name)) {
                    isAnimation=isVideoPlaying=true;
                    arrayBuffer = fr.result;
                    dataView = new DataView(arrayBuffer);
                    frameoffset=0;currentFrame=0;maxFrame=1;isAllFrameLoaded=false; 
    ...
    document.addEventListener("drop", (e) => {
        e.preventDefault();
        e.stopPropagation();
        //AnimationChunks = readChunksFromFile(file);
        //readChunksByID(); 
    ...
    function readChunksByID(){  
        if (frameoffset < dataView.byteLength) {
            if (frameoffset + 4 > dataView.byteLength) {
              throw new Error("Unexpected end of file while reading length");
            }
            
            const length = dataView.getUint32(frameoffset, false); // false = big-endian
            frameoffset += 4;
        
            if (frameoffset + length > dataView.byteLength) {
              throw new Error("Unexpected end of file while reading data chunk");
            }
        
            const chunk = arrayBuffer.slice(frameoffset, frameoffset + length);
            splatData = new Uint8Array(chunk);
            AnimationChunks.push(splatData);
            frameoffset += length;
            ...
            if (frameoffset >= dataView.byteLength){
                currentFrame=0;
                isAllFrameLoaded=true;
            }
            console.log("IN splatv splatData.length "+splatData.length +" frameoffset "+frameoffset ); 
        }else{
            currentFrame=0;
            isAllFrameLoaded=true; 
            console.log("frameoffset "+frameoffset+" > dataView.byteLength "+ dataView.byteLength);
        } 
    }
    ...
    if (!stopLoading) {
        if (isPly(splatData)) {
            ...
            if (/\.splatv$/i.test(url.href)) {
                isAnimation=isVideoPlaying=true;
                arrayBuffer = splatData.buffer;
                dataView = new DataView(arrayBuffer);
                frameoffset=0;currentFrame=0;maxFrame=1;isAllFrameLoaded=false; 
                console.log("dataView.byteLength "+dataView.byteLength);
                readChunksByID();
